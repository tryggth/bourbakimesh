...
echo "Filing R&D issue for WebGPU In-Browser Inference on ${REPO}..."

# Ensure labels exist
for label in ml ui rfc enhancement; do
  gh label create "$label" --repo "$REPO" --color "0e8a16" --description "Label $label" 2>/dev/null || true
done

# 1. Create Issue via gh CLI if not already existing
EXISTING_URL=$(gh issue list --repo "$REPO" --state open --search "In-Browser WebGPU" --json url --jq '.[0].url // empty')
if [ -n "$EXISTING_URL" ]; then
  ISSUE_URL="$EXISTING_URL"
  echo "Found existing WebGPU Issue: ${ISSUE_URL}"
else
  ISSUE_URL=$(gh issue create --repo "$REPO" \
    --title "feat(ml): R&D — In-Browser WebGPU & WebAssembly Latent MCTS Inference Engine" \
    --body "### Objective
...
- [ ] **Browser Benchmark Harness:** Minimal Vite/TypeScript harness measuring WebGPU vs Wasm sims/sec on CPU/GPU." \
    --label "ml,ui,rfc,enhancement")
  echo "Created WebGPU Issue: ${ISSUE_URL}"
fi

# 2. Refresh Open Tickets Inventory
mkdir -p reports
gh issue list --repo "$REPO" --state open --limit 50 \
  --json number,title,labels,url,body \
  --jq '.[] | "## Issue #\(.number): \(.title)\n**URL:** \(.url)\n**Labels:** \([.labels[].name] | join(", "))\n\n### Current Body\n\(.body)\n\n---\n"' > reports/open_tickets_inventory.md

echo "✅ WebGPU R&D ticket tracked, inventory refreshed."
