...
fn test_and_commutativity_proof_lifecycle() {
    ...
    let h1 = state
        .apply_step(&DeductionStep::AndElimR {
            hyp: "h0".to_string(),
        })
        .unwrap()
        .unwrap();
    ...
    let h2 = state
        .apply_step(&DeductionStep::AndElimL {
            hyp: "h0".to_string(),
        })
        .unwrap()
        .unwrap();
    ...
    let h3 = state
        .apply_step(&DeductionStep::AndIntro {
            left: "h1".to_string(),
            right: "h2".to_string(),
        })
        .unwrap()
    ...
    assert_eq!(
        state.hyps.get("h3"),
        Some(&Expr::And(Box::new(b), Box::new(a)))
    );
