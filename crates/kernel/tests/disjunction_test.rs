...
fn test_or_commutativity_via_or_elim() {
    ...
    let initial_hyps = vec![
        ...
        (
            "h1".to_string(),
            Expr::Impl(Box::new(a.clone()), Box::new(b_or_a.clone())),
        ),
        (
            "h2".to_string(),
            Expr::Impl(Box::new(b.clone()), Box::new(b_or_a.clone())),
        ),
    ];
...
fn test_ex_falso_and_disjunctive_syllogism() {
    ...
    let mut state = ProofState::new(vec![("h0".to_string(), Expr::False)], target.clone());
...
fn test_or_intro_and_contradiction_lifecycle() {
    ...
    let h4 = state
        .apply_step(&DeductionStep::OrIntroR {
            left: p.clone(),
            hyp: "h0".to_string(),
        })
        .expect("OrIntroR failed")
        .unwrap();
    assert_eq!(h4, "h4");
    assert_eq!(
        state.hyps.get("h4"),
        Some(&Expr::Or(Box::new(p.clone()), Box::new(p.clone())))
    );
...
fn test_or_elim_mismatched_branches_rejected() {
    ...
    let initial_hyps = vec![
        ...
        (
            "h2".to_string(),
            Expr::Impl(Box::new(b.clone()), Box::new(d.clone())),
        ), // Concludes D instead of C
    ];
