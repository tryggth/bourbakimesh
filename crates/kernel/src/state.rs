    ...
    if pattern == &Term::Var(var.to_string()) {
        ...
        match (pattern, actual) {
            ...
            (Term::Func(f1, a1), Term::Func(f2, a2)) => {
                ...
                a1.iter()
                    .zip(a2.iter())
                    .all(|(p, a)| find_term_witness(p, a, var, witness))
            }
            _ => false,
        }
    }
        ...
        match step {
            DeductionStep::AndElimL { hyp } => {
                let expr = self
                    .hyps
                    .get(hyp)
                    .ok_or_else(|| KernelError::HypothesisNotFound(hyp.clone()))?;
                match expr {
                    Expr::And(left, _) => {
            ...
            DeductionStep::AndIntro { left, right } => {
                let left_expr = self
                    .hyps
                    .get(left)
                    .ok_or_else(|| KernelError::HypothesisNotFound(left.clone()))?;
                let right_expr = self
                    .hyps
                    .get(right)
                    .ok_or_else(|| KernelError::HypothesisNotFound(right.clone()))?;
            ...
            DeductionStep::OrElim {
                ...
                let or_expr = self
                    .hyps
                    .get(hyp_or)
                    .ok_or_else(|| KernelError::HypothesisNotFound(hyp_or.clone()))?;
                ...
                let left_expr = self
                    .hyps
                    .get(left_impl)
                    .ok_or_else(|| KernelError::HypothesisNotFound(left_impl.clone()))?;
                ...
                let right_expr = self
                    .hyps
                    .get(right_impl)
                    .ok_or_else(|| KernelError::HypothesisNotFound(right_impl.clone()))?;
            ...
            DeductionStep::Contradiction { pos_hyp, neg_hyp } => {
                let pos_expr = self
                    .hyps
                    .get(pos_hyp)
                    .ok_or_else(|| KernelError::HypothesisNotFound(pos_hyp.clone()))?;
                let neg_expr = self
                    .hyps
                    .get(neg_hyp)
                    .ok_or_else(|| KernelError::HypothesisNotFound(neg_hyp.clone()))?;
                match neg_expr {
            ...
            DeductionStep::FalseElim { hyp_false } => {
                let expr = self
                    .hyps
                    .get(hyp_false)
                    .ok_or_else(|| KernelError::HypothesisNotFound(hyp_false.clone()))?;
                if *expr == Expr::False {
            ...
            DeductionStep::ModusPonens { r#impl, arg } => {
                let impl_expr = self
                    .hyps
                    .get(r#impl)
                    .ok_or_else(|| KernelError::HypothesisNotFound(r#impl.clone()))?;
                let arg_expr = self
                    .hyps
                    .get(arg)
                    .ok_or_else(|| KernelError::HypothesisNotFound(arg.clone()))?;
                match impl_expr {
            ...
            DeductionStep::ExistsIntro { hyp, var, body } => {
                ...
                if !find_expr_witness(body, hyp_expr, var, &mut witness) {
                    return Err(KernelError::TypeMismatch {
                        expected: format!(
                            "expression matching body {:?} with witness for {}",
                            body, var
                        ),
            ...
            DeductionStep::ExistsElim {
                ...
                let ex_expr = self
                    .hyps
                    .get(hyp_exists)
                    .ok_or_else(|| KernelError::HypothesisNotFound(hyp_exists.clone()))?;
                ...
                let impl_expr = self
                    .hyps
                    .get(hyp_impl)
                    .ok_or_else(|| KernelError::HypothesisNotFound(hyp_impl.clone()))?;
                ...
                if con.contains_free_var(ex_var)
                    || bound_var.map_or(false, |bv| con.contains_free_var(bv))
                {
                    return Err(KernelError::InvalidRuleApplication(format!(
                        "Variable {} occurs free in conclusion {:?}",
                        ex_var, con
                    )));
                }
            ...
            DeductionStep::Rewrite { eq_hyp, target_hyp } => {
                let eq_expr = self
                    .hyps
                    .get(eq_hyp)
                    .ok_or_else(|| KernelError::HypothesisNotFound(eq_hyp.clone()))?;
                ...
                let target_expr = self
                    .hyps
                    .get(target_hyp)
                    .ok_or_else(|| KernelError::HypothesisNotFound(target_hyp.clone()))?;
...
mod tests {
    ...
    fn test_modus_ponens_success_and_failure() {
        ...
        let mut state = ProofState::new(
            vec![("h0".to_string(), impl_expr), ("h1".to_string(), p.clone())],
            q.clone(),
        );
        ...
        assert_eq!(
            err2,
            Err(KernelError::HypothesisNotFound("h99".to_string()))
        );
    }
    ...
    fn test_or_intro_and_elim() {
        ...
        let mut state = ProofState::new(
            vec![
                ...
                (
                    "h1".to_string(),
                    Expr::Impl(Box::new(a.clone()), Box::new(c.clone())),
                ),
                (
                    "h2".to_string(),
                    Expr::Impl(Box::new(b.clone()), Box::new(c.clone())),
                ),
            ],
            c.clone(),
        );
        ...
        let h3 = state
            .apply_step(&DeductionStep::OrIntroL {
                hyp: "h0".to_string(),
                right: b.clone(),
            })
            .unwrap()
        ...
        assert_eq!(
            state.hyps.get("h3"),
            Some(&Expr::Or(Box::new(a.clone()), Box::new(b.clone())))
        );

        let h4 = state
            .apply_step(&DeductionStep::OrElim {
                hyp_or: "h3".to_string(),
    ...
    fn test_contradiction_and_false_elim() {
        ...
        let mut state = ProofState::new(
            vec![("h0".to_string(), p.clone()), ("h1".to_string(), not_p)],
            target,
        );

        let h2 = state
            .apply_step(&DeductionStep::Contradiction {
        ...
        let res = state
            .apply_step(&DeductionStep::FalseElim {
